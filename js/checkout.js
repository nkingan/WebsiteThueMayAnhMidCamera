...
document.addEventListener("DOMContentLoaded", () => {
    ...
    if (custPhone) {
        custPhone.addEventListener("input", () => custPhone.classList.remove("invalid-input"));
    }
    if (custCccd) {
        custCccd.addEventListener("input", () => custCccd.classList.remove("invalid-input"));
    }
});
...
function goToPaymentScreen() {
    ...
    const phoneVal = custPhone ? custPhone.value.trim() : "";
    const cccdVal = custCccd ? custCccd.value.trim() : "";

    // Reset validation styles
    if (custPhone) custPhone.classList.remove("invalid-input");
    if (custCccd) custCccd.classList.remove("invalid-input");
    ...
    if (!phoneRegex.test(phoneVal)) {
        errors.push("Số điện thoại không hợp lệ (phải nhập đúng 10 chữ số và bắt đầu bằng số 0).");
        if (custPhone) custPhone.classList.add("invalid-input");
    }

    if (!cccdRegex.test(cccdVal)) {
        errors.push("Số CCCD gắn chip không hợp lệ (phải nhập đúng 12 chữ số).");
        if (custCccd) custCccd.classList.add("invalid-input");
    }
